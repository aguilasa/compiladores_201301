...
public class Principal {
	...
	private final boolean LOOKLIKEWIN = true;
	...
	private void initialize() {
		...
		groupLayout.setHorizontalGroup(
			groupLayout.createParallelGroup(Alignment.LEADING)
				.addComponent(toolBar, GroupLayout.DEFAULT_SIZE, 784, Short.MAX_VALUE)
				.addComponent(status, GroupLayout.DEFAULT_SIZE, 784, Short.MAX_VALUE)
		...
		addButton(manager, "novo", "novo [ctrl-n]", KeyEvent.VK_N,
				InputEvent.CTRL_MASK, ac);
		addButton(manager, "abrir", "abrir [ctrl-a]", KeyEvent.VK_A,
				InputEvent.CTRL_MASK, ac);
		addButton(manager, "salvar", "salvar [ctrl-s]", KeyEvent.VK_S,
				InputEvent.CTRL_MASK, ac);
		addButton(manager, "copiar", "copiar [crtl-c]", KeyEvent.VK_C,
				InputEvent.CTRL_MASK, new CopyAction());
		addButton(manager, "colar", "colar [ctrl-v]", KeyEvent.VK_V,
				InputEvent.CTRL_MASK, new PasteAction());
		addButton(manager, "recortar", "recortar [ctrl-x]", KeyEvent.VK_X,
				InputEvent.CTRL_MASK, new CutAction());
		addButton(manager, "compilar", "compilar [F8]", KeyEvent.VK_F8, 0, ac);
		addButton(manager, "gerar", "gerar código [F9]", KeyEvent.VK_F9, 0,
				ac);
		addButton(manager, "equipe", "equipe [F1]", KeyEvent.VK_F1, 0,
				ac);

		frame.getContentPane().setLayout(groupLayout);
	}

	private void windowsLike() {
		UIManager.LookAndFeelInfo[] lookAndFeelInfo = UIManager
				.getInstalledLookAndFeels();
		String className = "";
		for (int i = 0; i < lookAndFeelInfo.length; i++) {
			if (lookAndFeelInfo[i].getName().equalsIgnoreCase("windows")) {
